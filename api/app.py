...
def save_audit_data(ip_address, action):
    with open("audit_log.txt", "a") as file:
        file.write(
            f"IP Address: {ip_address} - Action: {action} - Time: {datetime.now().isoformat()}\n"
        )
...
def get_client_ip():
    if request.headers.getlist("X-Forwarded-For"):
        ...
        ip_address = request.headers.getlist("X-Forwarded-For")[0].split(
            ",")[0].strip()
    else:
...
def post(post_id):
    ...
    if post:
        post['content'] = markdown.markdown(
            post['content'], extensions=['markdown.extensions.fenced_code'])

        if "\n" in post['content']:
            post_content = post['content'].replace("\n", "<br>")

        else: post_content = post['content']
            
        ip_address = get_client_ip()
        save_audit_data(ip_address, f"Accessed post {post_id}")
        return render_template("post.html", post=post, post_content = post_content, post_ammount = len(post['comments']))
